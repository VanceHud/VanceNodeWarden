import { User, Cipher, Folder, Attachment, Device, Send } from '../types';
...
const SCHEMA_STATEMENTS: readonly string[] = [
  ...
  'key TEXT NOT NULL, password_hash TEXT, password_salt TEXT, password_iterations INTEGER, ' +
...
export class StorageService {
  ...
  async getSend(id: string): Promise<Send | null> {
    const row = await this.db
      .prepare(
        'SELECT id, user_id, type, name, notes, data, key, password_hash, password_salt, password_iterations, max_access_count, access_count, disabled, hide_email, created_at, updated_at, expiration_date, deletion_date FROM sends WHERE id = ?'
      )
      .bind(id)
      .first<any>();
    if (!row) return null;
  ...
  async saveSend(send: Send): Promise<void> {
    const stmt = this.db.prepare(
      'INSERT INTO sends(id, user_id, type, name, notes, data, key, password_hash, password_salt, password_iterations, max_access_count, access_count, disabled, hide_email, created_at, updated_at, expiration_date, deletion_date) ' +
      'VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ' +
      'ON CONFLICT(id) DO UPDATE SET ' +
  ...
  async getAllSends(userId: string): Promise<Send[]> {
    const res = await this.db
      .prepare(
        'SELECT id, user_id, type, name, notes, data, key, password_hash, password_salt, password_iterations, max_access_count, access_count, disabled, hide_email, created_at, updated_at, expiration_date, deletion_date FROM sends WHERE user_id = ? ORDER BY updated_at DESC'
      )
      .bind(userId)
      .all<any>();
  ...
  async getSendsPage(userId: string, limit: number, offset: number): Promise<Send[]> {
    const res = await this.db
      .prepare(
        'SELECT id, user_id, type, name, notes, data, key, password_hash, password_salt, password_iterations, max_access_count, access_count, disabled, hide_email, created_at, updated_at, expiration_date, deletion_date FROM sends WHERE user_id = ? ORDER BY updated_at DESC LIMIT ? OFFSET ?'
      )
      .bind(userId, limit, offset)
      .all<any>();
    return (res.results || []).map(row => this.mapSendRow(row));
